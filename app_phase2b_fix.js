(() => {
  ...
  function hardWireCoreButtons() {
    ...
    if (input && !input.dataset.xalvionBound) {
      ...
      input.dataset.xalvionBound = "true";
    }
  }
  ...
  async function safeInit() {
    try {
      ...
      console.error("Xalvion init failed", error);
      hardWireCoreButtons();
      setSending(false);
      addEmptyState();
      setNotice("error", "Workspace recovered", "The workspace hit an init error, but core send controls were restored. Refresh if this keeps happening.");
    }
  }
